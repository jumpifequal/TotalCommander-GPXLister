...
**Latest release:** `v2.8.2` improves the hovering circle betweekn 2D and 3D and keep the zoom level on the track consistent between 2D and 3D.
...
- `hoverCircleRadius` (float, default `6.0`): DPI-aware logical radius shared by the 2D and 3D hover marker, clamped to `2.0` through `30.0`.
